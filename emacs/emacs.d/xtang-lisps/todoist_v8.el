...
(defun todoist--query (method &optional data)
...
DATA is the request body."
  (let ((url todoist-url)
        ...
        (url-request-data (json-encode (append (json-read-from-string data)
                                               '(("token". "3c5d950a279c4fdd885833b5fa23ecce523ac070"))))))
...
(defun todoist--filter-sections (project sections)
  "Get subset of sections under a project.

PROJECT the project.
TASKS the list of sections."
  (-filter (lambda (section)
             (equal (todoist--section-project-id section) (todoist--project-id project)))
           sections))
...
(defun  todoist--filter-tasks-by-section (section tasks)
  (-filter (lambda (task)
...
TODO is boolean to show TODO tag."
  ...
  (org-set-property "TODOIST_SECTION_ID" (format "%s" (todoist--task-section-id task)))
  (org-set-property "TODOIST_PROJECT_ID" (format "%s" (todoist--task-project-id task))))

(defun todoist--insert-section (section tasks)
  (todoist--insert-heading 3 (todoist--section-name section))
  (mapcar (lambda (task) (todoist--insert-task task 4 nil))
          (todoist--filter-tasks-by-section section tasks))
  )
...
TASKS the list of all tasks."
  ...
  (mapcar (lambda (section) (todoist--insert-section section (todoist--filter-tasks project tasks)))
          (todoist--filter-sections project sections)))
...
CACHE to read from cache rather than query upstream."
  (if cache
    ...
    (setq todoist--cached-projects
          (append
           (assoc-default 'projects
                          (todoist--query "POST"
                                          (json-encode `(("resource_types" . ["projects"])))))
           nil))))

(defun todoist--get-sections ()
  "Get the list of all tasks."
  (append (assoc-default 'sections
                         (todoist--query "POST" (json-encode `(("resource_types" . ["sections"])))))
          nil))

(defun todoist--get-tasks ()
  "Get the list of all tasks."
  (append (assoc-default 'items
                         (todoist--query "POST" (json-encode `(("resource_types" . ["items"])))))
          nil))
